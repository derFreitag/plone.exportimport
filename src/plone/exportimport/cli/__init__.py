...
def partial_exporter_cli(args=sys.argv):
    ...
    breakpoint()
    with api.env.adopt_roles(["Manager"]):
        results = get_exporter(site).partial_export_site(path, paths_list=paths_list, options=namespace)
    logger.info(f" Using path {path} to export partial content from Plone site at /{site.id}")
    for item in results[1:]:
        logger.info(f" Wrote {item}")
